        ...
        document.querySelector(".cardContainer").innerHTML += `
            <div class="col-md-3">
                <div class="card shadow card${i}">
                    <div class="cardImage">
                        <img src="sana-sadinaz-image/${items[i].itemImage}" alt="${items[i].itemName}">
                    </div>
                    <div class="cardContent">
                        <h3 class="colour">${items[i].itemName}</h3>
                        <!-- Button to trigger the download -->
                        <button class="but" type="button" onclick="downloadFile('sana-sadinaz-docx/${items[i].itemdocx}')">Learn More</button>
                    </div>
                </div>
            </div>
        `;
